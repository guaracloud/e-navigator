...
| --- | --- | --- | --- | --- | --- | --- |
| Unified source supervisor | Default unified mode registers enabled general and CPU sources together; deployed `isolate` policy preserves healthy sources; Linux sources handle SIGTERM and bounded sink drain; one node controller feeds all filter maps and production attribution; a bounded native registry exposes per-source running state and lifecycle totals | Unit, synthetic, lifecycle, partial-failure, source-health, shared snapshot, and process-exclusion tests | attachment-level health | Combined live capture+profile, partial-attach, and termination proof | supervisor | All enabled real sources in one process; injected source failure leaves healthy source running under `isolate` |
...
| Self-observability | Source accounting logs and warnings; Prometheus signal rendering; native per-source running/start/exit/failure metrics; native controller readiness/freshness/relist/watch/reconciliation/cgroup metrics; native exporter queue/retry/circuit/drop metrics | Unit, source lifecycle, registry integration, and selected logs | attachment health plus protocol/TLS/profile/process metrics | alert and failure-injection proof | health | every induced blind spot changes a documented native metric/health state |
