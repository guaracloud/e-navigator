...
| Performance and soak | Criterion hot paths, resumable soak collection, and a guarded cumulative no-agent/reference/E-Navigator comparison harness | Local microbenchmarks and a corrected per-stage protocol completeness gate. The earlier 33-run homelab comparison is invalidated because its Redis connection predated collector attachment | valid replacement CPU, RSS, allocation, throughput, and latency evidence plus production tuning thresholds | 24-hour minimum data and production-shaped capacity proof | evidence | every offered signal family is proven before comparison, negative results remain explicit, and manifests/checksums preserve provenance |
