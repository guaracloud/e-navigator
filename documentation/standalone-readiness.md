...
| --- | --- | --- | --- | --- | --- | --- |
| Unified source supervisor | Default unified mode registers enabled general and CPU sources together; deployed `isolate` policy preserves healthy sources; Linux sources handle SIGTERM and bounded sink drain | Unit, synthetic, lifecycle and partial-failure tests | shared controller state and source health registry | Combined live capture+profile, partial-attach, and termination proof | supervisor | All enabled real sources in one process; injected source failure leaves healthy source running under `isolate` |
...
| Export isolation | Independent metric/trace/profile workers with bounded queues, size/time batches, jittered retries, circuit breaking and shutdown drain | Deterministic fake HTTP failure/overload/recovery tests and local Pyroscope smoke | production telemetry registry wiring | Destination failure, saturation, memory-bound and cross-family latency proof | exporters | unavailable profile endpoint does not change trace/metric latency; memory remains bounded |
