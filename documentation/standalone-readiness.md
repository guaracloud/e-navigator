...
| Protocol and TLS | HTTP/1, h2/gRPC, PostgreSQL, MySQL, Redis, MongoDB, Kafka, and NATS parsers; bounded same-stream HTTP/2 HEADERS/CONTINUATION reassembly before HPACK decoding; version-gated, architecture-checked, export-preflighted, transactional uprobes for dynamically linked OpenSSL 1.1.1/3 and GnuTLS ABI 30 standard socket transports; separate read/write fd tracking; fail-closed unknown/incomplete ABI handling; native optional-target metrics | Fixtures/fuzz-build, local HTTP/2 continuation and fail-closed state tests, live OpenSSL 3 and GnuTLS ABI 30 HTTP/1, local fail-closed unknown-ABI proof, and selected older live protocols | server-side breadth and remaining correlation gaps; BoringSSL, Go/rustls, custom BIO/transports, and statically bundled Node/JVM TLS are explicit non-goals for this claimed surface | homelab matrix for both claimed library families and every claimed application protocol | protocol-tls | automated request/status/secret-loss assertions per claimed protocol/runtime, with unsupported libraries visibly rejected |
