...
| Process exec source | yes | no | userspace config and decode tests | guarded homelab runs `20260619-205746`, `20260619-210812-extended`, and `20260620-180718-failed-inconclusive-validation` observed `source.aya_exec` records from live Kubernetes nodes | reduced privilege hardening, workload-attributed benchmark capture |
| TCP network source | yes | no | raw decode tests | guarded homelab runs `20260619-205746`, `20260619-210812-extended`, and `20260620-180718-failed-inconclusive-validation` observed `source.aya_network` records from live Kubernetes nodes | full TCP state, packet accounting, workload-attributed benchmark capture |
...
| CPU profiling source | yes, explicit opt-in source | no | raw decode, profile normalization, generator tests | homelab `aya-cpu-profile` canaries observed `source.aya_cpu_profile` sample envelopes and bounded raw IP stack frames, including `20260620-180718-failed-inconclusive-validation`; controlled CPU-workload attribution is not proven for the 20260620 run | function symbolization, controlled workload attribution, pprof, Pyroscope, OTLP profiles, flamegraph UI, storage |
...
| Exporter infrastructure | partial | no | sink-layer fake-collector tests for batching, timeout config, retry, bounded queue, headers, and drop counters; `20260619-210812-extended` and `20260620-180718-failed-inconclusive-validation` confirmed only `sink.json_stdout` is registered in this release | no | full OTLP protobuf metrics/traces/profiles, production collector compatibility proof |
| Benchmark evidence harness | yes | no | local Criterion compile and smoke runs for deterministic parser, decode, generator, formatter, and queue fixtures | guarded homelab runs `20260619-205746`, `20260619-210812-extended`, and `20260620-180718-failed-inconclusive-validation` recorded raw DaemonSet, pod, log, event, workload, capability, resource, and metrics artifacts | committed raw results, live overhead baselines, homelab runtime comparison |
