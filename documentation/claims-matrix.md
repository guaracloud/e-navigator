...
| Process exec source | yes | no | userspace config and decode tests | guarded homelab runs `20260619-205746` and `20260619-210812-extended` observed `source.aya_exec` records from live Kubernetes nodes | reduced privilege hardening, workload-attributed benchmark capture |
| TCP network source | yes | no | raw decode tests | guarded homelab runs `20260619-205746` and `20260619-210812-extended` observed `source.aya_network` records from live Kubernetes nodes | full TCP state, packet accounting, workload-attributed benchmark capture |
| Runtime DNS capture | no | yes, DNS envelopes and fixtures | schema/generator/smoke tests | no | eBPF DNS packet capture |
| Host resource source | yes | no | procfs/sysfs/cgroup parser tests and Docker synthetic fixtures | guarded homelab runs `20260619-205746` and `20260619-210812-extended` observed `source.host_resource` node, process, and cgroup records from mounted host paths | warning detail expansion, longer host accuracy baseline |
...
| Exporter infrastructure | partial | no | sink-layer fake-collector tests for batching, timeout config, retry, bounded queue, headers, and drop counters; `20260619-210812-extended` confirmed only `sink.json_stdout` is registered in this release | no | full OTLP protobuf metrics/traces/profiles, production collector compatibility proof |
| Benchmark evidence harness | yes | no | local Criterion compile and smoke runs for deterministic parser, decode, generator, formatter, and queue fixtures | guarded homelab runs `20260619-205746` and `20260619-210812-extended` recorded raw DaemonSet, pod, log, event, workload, capability, resource, and metrics artifacts | committed raw results, live overhead baselines, homelab runtime comparison |
