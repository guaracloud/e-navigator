...
- bounded DNS request parsing with configurable packet and diagnostic preview
  ...
  MySQL command packet parsing for query/ping/prepare/execute plus OK/EOF/ERR
  ...
  PostgreSQL Query/Parse/Execute/Flush/Sync wire-message and
...
- **MySQL protocol observability:** bounded `COM_QUERY`,
  `COM_PING`, `COM_STMT_PREPARE`, `COM_STMT_EXECUTE`, and OK/EOF/ERR response
  parsing is locally tested without exporting raw SQL text, statement IDs, parameter
...
- **PostgreSQL protocol observability:** bounded simple Query, Parse, Execute,
  Flush, Sync, CommandComplete, ReadyForQuery, and ErrorResponse parsing is
  locally tested without exporting raw SQL text or raw error messages, including canonical
