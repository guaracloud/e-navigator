...
- bounded DNS request parsing with configurable packet and diagnostic preview
  ...
  ParseComplete/BindComplete/CloseComplete/RowDescription/DataRow/NoData/
...
- **PostgreSQL protocol observability:** bounded simple Query, Parse, Bind,
  ...
  CloseComplete, RowDescription, DataRow, NoData, CommandComplete,
  NoticeResponse, ParameterStatus, ReadyForQuery, and ErrorResponse parsing is
  locally tested without exporting raw SQL text, function OIDs, argument values,
  row values, copy payloads, copy failure text, password values, row field names,
