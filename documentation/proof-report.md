...
- bounded DNS request parsing with configurable packet and diagnostic preview
  ...
  nested-array/RESP3-map/RESP3-set/error response parsing with declared
  frame-length bounds,
...
- **Redis protocol observability:** bounded RESP command and
  simple/integer/bulk/RESP3-scalar/RESP3-blob-error/verbatim/flat-array/
  nested-array/RESP3-map/RESP3-set/error response parsing is locally tested
  without exporting raw key/value payloads or raw error messages, including
  declared frame-length bounds and bounded response-status token validation, but
  runtime capture, request/response matching, broad response coverage, and live
  Redis proof are not implemented or proven.
