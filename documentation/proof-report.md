...
- bounded DNS request parsing with configurable packet and diagnostic preview
  ...
  Kafka request-header plus bounded ApiVersions and non-flexible Metadata
  request bodies, ApiVersions response, bounded Produce request/response, and
  bounded non-flexible Fetch and Metadata request/response-error parsing,
  MongoDB
...
- **Kafka protocol observability:** bounded request-header parsing for common
  ...
  Produce, Fetch, and Metadata request-body validation, and ApiVersions,
  Produce, and non-flexible Fetch and Metadata response-error parsing is locally
  tested without exporting client IDs, software names, topics, record payloads,
  broker hosts, cluster IDs, or response body values, but runtime capture,
