...
- bounded DNS request parsing with configurable packet and diagnostic preview
  ...
  Heartbeat, LeaveGroup, ListGroups, ListOffsets, and Metadata request bodies,
  ApiVersions response, bounded Produce request/response, and bounded
  non-flexible Fetch, FindCoordinator, Heartbeat, LeaveGroup, ListGroups,
...
- **Kafka protocol observability:** bounded request-header parsing for common
  ...
  ListOffsets, and Metadata request-body validation, and ApiVersions, Produce,
  and non-flexible Fetch, FindCoordinator, Heartbeat, LeaveGroup, ListGroups,
  ListOffsets, and Metadata response-error parsing is locally tested without
  exporting client IDs, coordinator keys, consumer group/member identifiers,
  software names, topics, record payloads, broker hosts, cluster IDs, or
