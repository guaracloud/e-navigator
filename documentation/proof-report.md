...
- strict config validation with unknown-field rejection, packaged config guards,
  ...
  OTLP HTTP sink runtime-bound validation, OTLP HTTP endpoint-shape/length
  validation, local Kubernetes attribution selector filtering and
  selector-shape bounds plus response/cache/label bounds, and host resource
  source scan plus
