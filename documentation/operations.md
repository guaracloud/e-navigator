...
| HTTP propagation does not inject | Plaintext allowlisted port, active socket established after attachment, one complete HTTP/1 header block in the contiguous captured prefix, exact `Content-Length` or a chunked current syscall whose every byte is captured and structurally valid, no visible/provable trailing request, at most 40 length-accounted iovecs with headers captured in the first three 96-byte prefixes, no existing/orphan context, same-thread inbound continuation, cgroup capture verdict, context-pool and propagation counters including `unsupported_iovec` and planner rejection |
