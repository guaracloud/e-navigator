...
large exact-`Content-Length` tails, fully captured bounded chunked framing, and
exact total-length accounting through 40 iovecs, while capturing only the first three
...
ambiguous state fail closed. Redis ordinary RESP3 pushes/attributes stay out of
FIFO matching. The three RESP2 Pub/Sub delivery shapes do so only after a
request-matched confirmation establishes RESP2 subscriber state for the
connection; identical arrays without that evidence remain replies. Explicit
subscription commands retain only their bounded confirmation count, and a
matched `RESET` exits subscriber mode. Zero-argument unsubscribe forms make
the connection opaque rather than guessing when their acknowledgement stream
ends across mixed subscription kinds.
...
| 4 | Finish HTTP/1 wire framing beyond the bounded large/fully-captured-chunked slice | Model/property/differential tests, verifier matrix, segmented-header and pipelined-request live-wire tests, privileged proof for current large/fully-captured-chunked forms, mutation failure injection | Kernel/proxy diversity and mutation blast radius |
