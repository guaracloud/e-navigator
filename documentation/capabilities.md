...
| Prometheus HTTP sink | Implemented opt-in HTTP surface with network, DNS, resource, and profile session aggregate rendering | local `/metrics`, `/healthz`, `/readyz`, and profile session aggregate rendering tests plus selected live scrape proof | longer soak, cardinality baseline, production-load correctness |
