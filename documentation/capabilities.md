...
| CPU profiling | Implemented Aya perf-event CPU sampling with configurable stack depth (default 64, up to 128 frames, kernel-sysctl bounded) with per-sample truncation flags and capped-depth accounting, pid-namespace-aware pid translation with identity-verified fallback, procfs-maps + bounded ELF symbolization (module, module-relative offset, best-effort local function names), source-layer drop and coverage-cap accounting, frequency-sized per-CPU perf rings, session aggregation, pprof rendering with real addresses/mappings, a `/debug/pprof/profile` serving endpoint, and OTLP profile export | source/generator/symbolizer/sink tests, raw-event and symbolize fuzz targets, and local OrbStack live runs resolving real modules and serving pprof, including a 2026-07-05 run capturing fully symbolized 85-frame deep-recursion stacks and flagged-plus-counted truncation under a 16-frame budget | DWARF/CFI stack unwinding for frame-pointer-omitted binaries and interpreted/JIT frames, live Kubernetes-node symbolization, production overhead baselines |
