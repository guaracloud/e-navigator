...
| HTTP/request foundation | Partial opt-in cleartext HTTP/1 client capture plus decoded gRPC-over-HTTP/2 metadata and trailer-status parser foundations | fixture tests, gRPC metadata/status parser tests, synthetic request/span coverage, and selected homelab `writev`/bounded-iovec proof | TLS, live HTTP/2 frame/HPACK capture, live gRPC capture/status matching, inbound server-side parsing, live HTTP status codes, route templates, broad multi-iovec support |
