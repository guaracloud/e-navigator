...
| `source.aya_http` | Opt-in | Captures bounded cleartext outbound HTTP request bytes from write-style syscall paths, parses request method/target metadata and W3C trace context when present, and emits protocol request observations. |
| `source.aya_protocol` | Opt-in | Captures bounded socket payload prefixes for outbound TCP connections to configured Kafka, MongoDB, MySQL, NATS, PostgreSQL, and Redis ports, reassembles per-connection request streams with explicit truncation/desync accounting, and emits protocol request observations through the existing bounded parsers. |
| `source.aya_cpu_profile` | Opt-in source mode and config | Samples CPU profiles through Aya perf events, bounds active CPUs, frames, symbols, modules, files, and batch size, and emits profile sample observations with stack ids and instruction-pointer frames. |
...
| Protocol runtime capture | Implemented opt-in `source.aya_protocol` socket payload capture for outbound client connections to configured protocol ports, with bounded per-connection stream reassembly, pipelined-frame extraction, and drop/desync/truncation accounting feeding the existing parsers | stream decoder and registry tests, raw-event and stream fuzz targets, Criterion reassembly benchmarks, and a local OrbStack Docker live Redis capture run (10 pipelined RESP observations, no key export) | server-side/inbound capture, read-direction response decoding, request/response matching, truncated-frame prefix parsing for oversized frames, TLS, live proof for Kafka/PostgreSQL/MySQL/MongoDB/NATS, production overhead baselines |
