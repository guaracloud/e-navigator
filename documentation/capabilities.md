...
| HTTP/request foundation | Partial opt-in cleartext HTTP/1 client capture plus bounded HTTP/1 response-status and decoded gRPC-over-HTTP/2 metadata/trailer-status parser foundations | fixture tests, HTTP response-status parser tests, gRPC metadata/status parser tests, synthetic request/span coverage, and selected homelab `writev`/bounded-iovec proof | TLS, live HTTP/2 frame/HPACK capture, live HTTP/gRPC status matching, inbound server-side parsing, route templates, broad multi-iovec support |
