...
| HTTP/request foundation | Partial opt-in cleartext HTTP/1 client capture plus decoded gRPC-over-HTTP/2 metadata parser foundation | fixture tests, gRPC metadata parser tests, synthetic request/span coverage, and selected homelab `writev`/bounded-iovec proof | TLS, live HTTP/2 frame/HPACK capture, live gRPC capture/status extraction, inbound server-side parsing, status codes, route templates, broad multi-iovec support |
