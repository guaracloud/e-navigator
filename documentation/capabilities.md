...
| Kafka parser foundation | Parser-only request-header extraction for bounded API-key semantics without client id, topic, or payload export | parser fixture, malformed-input, property, and fuzz-target coverage | runtime eBPF capture, request/response matching, status/error extraction, flexible-version body semantics, live Kafka proof |
