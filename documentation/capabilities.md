...
| Redis parser foundation | Parser-only RESP command and error-response extraction for bounded command/status semantics without raw key/value or raw error-message export | parser fixture, malformed-input, property, fuzz-target, synthetic request/span coverage, and local error-status trace export tests | runtime eBPF capture, request/response matching, broad response coverage, live Redis proof |
