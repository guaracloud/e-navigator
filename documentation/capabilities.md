...
| `source.aya_protocol` | Opt-in | Captures bounded socket payload prefixes in both directions for outbound TCP connections to configured Kafka, MongoDB, MySQL, NATS, PostgreSQL, and Redis ports, reassembles per-connection streams with explicit truncation/desync accounting, matches responses to a bounded in-flight request queue for real latency and response-status semantics, and emits protocol request observations through the existing bounded parsers. |
