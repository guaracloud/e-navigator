...
| CPU profiling | Partial opt-in CPU profile sampling plus bounded session aggregation and dropped-sample warnings | raw decode/profile tests, local dropped-sample warning tests, OTLP profile sample workload-identity tests, and selected live workload attribution | pprof export, storage, symbolization, flamegraphs, deterministic all-workload capture |
