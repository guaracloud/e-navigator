...
| `source.aya_tls` | Opt-in | Captures TLS plaintext at the userspace library boundary via uprobes on OpenSSL/BoringSSL `SSL_read`/`SSL_write` and GnuTLS `gnutls_record_recv`/`gnutls_record_send` (library-boundary interception, not on-the-wire decryption). Maps each TLS handle to its socket fd through `SSL_set_fd`/`gnutls_transport_set_ptr`, reuses the connect/accept connection tuples, emits the same configurable multi-segment window as `source.aya_protocol`, and routes decrypted HTTP/2 and database plaintext through the same bounded reassembler, parsers, and request/response matcher. Libraries lacking a probed symbol are accounted, not silently missed. HTTP/1-over-TLS framing is not yet implemented. |
