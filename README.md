...
- dependency graph, resource metrics, network metrics with precise duplicate
  ...
  target, HTTP response-status, configurable bounded HTTP parser limits, gRPC
  metadata/status, Kafka request/ApiVersions-error, MongoDB command/error, MySQL
  command/error, NATS command/error, PostgreSQL query/error, and Redis
