...
- dependency graph, resource metrics, network metrics, configurable bounded DNS
  source limits, HTTP response-status, configurable bounded HTTP parser limits,
  gRPC metadata/status, Kafka request/ApiVersions-error, MongoDB command/error,
  MySQL command/error, NATS command/error, PostgreSQL query/error, and Redis
