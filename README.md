...
- dependency graph, resource metrics, network metrics, HTTP response-status,
  ...
  parser foundations, request/trace/profile foundations with OTLP HTTP and
  gRPC trace status mapping and warning trace-record formatting, and runtime
  security generator behavior through tests, including flow-attribution
  warnings;
