...
results_root="${E_NAVIGATOR_PROFILING_RESULTS_DIR:-benchmarks/results/profiling-breadth-proof}"
duration_seconds="${E_NAVIGATOR_PROFILING_DURATION_SECONDS:-60}"
repetitions="${E_NAVIGATOR_PROFILING_REPETITIONS:-3}"
image_repository="${E_NAVIGATOR_HOMELAB_IMAGE_REPOSITORY:-docker.io/library/e-navigator}"
image_tag="${E_NAVIGATOR_HOMELAB_IMAGE_TAG:-gap4-dev-amd64}"
...
if [ "${E_NAVIGATOR_HOMELAB_CONFIRM:-0}" != "1" ]; then
  printf 'refusing profiling proof without E_NAVIGATOR_HOMELAB_CONFIRM=1\n' >&2
  exit 2
fi
if [ "$context" != "homelab" ] || [ "$namespace" != "e-navigator-bench" ]; then
  printf 'profiling proof target must be exactly homelab/e-navigator-bench\n' >&2
  exit 2
fi
case "$duration_seconds" in
  ""|*[!0-9]*)
    printf 'E_NAVIGATOR_PROFILING_DURATION_SECONDS must be an integer\n' >&2
    exit 2
    ;;
esac
if [ "$duration_seconds" -lt 30 ] || [ "$duration_seconds" -gt 300 ]; then
  printf 'profiling proof duration must be between 30 and 300 seconds\n' >&2
  exit 2
fi
case "$repetitions" in
  1|2|3|4|5) ;;
  *)
    printf 'E_NAVIGATOR_PROFILING_REPETITIONS must be between 1 and 5\n' >&2
    exit 2
    ;;
esac
...
run_arm() {
  ...
  E_NAVIGATOR_HOMELAB_CONFIG_TEMPLATE=benchmarks/config/profiling-breadth.toml \
  ...
  E_NAVIGATOR_HOMELAB_WORKLOAD_TEMPLATE=benchmarks/k8s/profiling-breadth-workload.yaml \
  E_NAVIGATOR_HOMELAB_WORKLOAD_DURATION_SECONDS="$duration_seconds" \
  E_NAVIGATOR_HOMELAB_TOP_SAMPLES=10 \
  E_NAVIGATOR_HOMELAB_TOP_INTERVAL_SECONDS=5 \
  E_NAVIGATOR_HOMELAB_LOG_TAIL=100000 \
  ...
  python3 benchmarks/runner/analyze-profiling-breadth.py \
    --validate-run "$mode" "$run_dir" >"$run_dir/validated-run.json"
...
python3 benchmarks/runner/analyze-profiling-breadth.py "$results_root" \
