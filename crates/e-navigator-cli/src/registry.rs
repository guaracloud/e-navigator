...
pub(crate) fn build_registry(
    ...
    if config.module_enabled("generator.request_correlation") {
        registry = registry.with_generator(Box::new(RequestCorrelationGenerator::from_config(
...
fn aya_source_telemetry_lines(
    ...
    snapshots
        .flat_map(|snapshot| {
            let source_labels = std::collections::BTreeMap::from([(
                "source".to_string(),
            ...
            let mut transport_labels = source_labels;
            transport_labels.insert(
                "transport".to_string(),
