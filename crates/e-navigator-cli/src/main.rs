...
use e_navigator_signals::{
    ...
    ProcessExitEvent, ProcessResourceContext, ProcessResourceObservation, ProfilingAttribute,
    ProfilingConfidence, ProfilingCorrelationKind, ProfilingKind, ProtocolKind,
...
impl Source<SignalEnvelope> for SyntheticExecSource {
    ...
    async fn run(self: Box<Self>, tx: mpsc::Sender<SignalEnvelope>) -> CoreResult<()> {
        ...
        for signal in synthetic_profile_signals(
            self.host.clone(),
            container.clone(),
            kubernetes.clone(),
